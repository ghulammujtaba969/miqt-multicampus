...
function getAdvancedRuleFromRequest($index, $allowedFields, $allowedOperators) {
...
function appendAdvancedRule(&$whereParts, &$params, $rule, $allowedFields) {
...
$ruleOne = getAdvancedRuleFromRequest(1, $allowedAdvancedFields, $allowedOperators);
$ruleTwo = getAdvancedRuleFromRequest(2, $allowedAdvancedFields, $allowedOperators);

if ($ruleOne['field'] || $ruleTwo['field']) {
    $showAdvanced = '1';
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign_class_bulk'])) {
    $assignClassId = isset($_POST['assign_class_id']) && $_POST['assign_class_id'] !== '' ? (int) $_POST['assign_class_id'] : null;
    ...
    if (empty($selectedStudents)) {
        ...
        try {
            ...
            logActivity($db, 'Bulk Assign Classes', 'Students', 'Assigned ' . count($selectedStudents) . ' students to class: ' . $assignedClassLabel);
            setFlash('success', count($selectedStudents) . ' student(s) updated successfully.');
            redirect(SITE_URL . '/modules/students/bulk_assign_classes.php?' . http_build_query($_GET));
        } catch (PDOException $e) {
            $error = 'Error assigning class: ' . $e->getMessage();
        }
    }
}
...
if ($unassignedOnly === '1') {
    $whereParts[] = "s.class_id IS NULL";
}

$advancedWhereParts = [];
$advancedParams = [];
$hasRuleOne = appendAdvancedRule($advancedWhereParts, $advancedParams, $ruleOne, $allowedAdvancedFields);
$hasRuleTwo = appendAdvancedRule($advancedWhereParts, $advancedParams, $ruleTwo, $allowedAdvancedFields);
...
$sql = "SELECT s.*, c.class_name
        ...
        $whereSql
        ORDER BY
            CASE WHEN s.class_id IS NULL THEN 0 ELSE 1 END,
            c.class_name ASC,
            s.first_name ASC,
            s.last_name ASC,
            s.id DESC";
...
<div class="page-header">
    <h1 class="page-title"><i class="fas fa-layer-group"></i> Bulk Assign Student Classes</h1>
    <p class="subtitle">Search students, select multiple records, and assign a class in one step</p>
</div>
    ...
    <div class="card-body">
        <form method="GET" action="">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    <label for="class">Search Class</label>
                    <select name="class" id="class" class="form-control">
                        <option value="">All Classes</option>
                        <?php foreach ($classes as $class): ?>
                        <option value="<?php echo $class['id']; ?>" <?php echo ($classFilter === (int) $class['id']) ? 'selected' : ''; ?>>
                ...
                <div class="form-group bulk-assign-actions">
                    ...
                    <a href="bulk_assign_classes.php" class="btn btn-secondary">
            ...
            <div id="advancedSearchPanel" class="advanced-search-panel <?php echo ($showAdvanced === '1') ? 'is-visible' : ''; ?>">
                <div class="advanced-search-header">
                    ...
                    <p>Filter by student table fields, including empty values like missing last name or date of birth.</p>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group advanced-checkbox-group">
                        <label class="checkbox-label">
                            ...
                            Show only students with no class assigned
                        </label>
                    </div>
                </div>
...
<div class="dashboard-card mb-20">
    <div class="card-body">
        <form method="POST" action="">
            <div class="form-row">
                <div class="form-group">
                    <label for="assign_class_id">Assign Selected Students To Class</label>
                    <select name="assign_class_id" id="assign_class_id" class="form-control" required>
                ...
                <div class="form-group bulk-assign-actions">
                    <button type="submit" name="assign_class_bulk" class="btn btn-success">
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="10" class="text-center text-muted">No students found for the current filters.</td>
                        </tr>
                        <?php else: ?>
